...
- Codex configs (`config.toml`, `rules/`, `memories/`) are symlinked to `~/.codex/`, each TOML file in `config/codex/agents/` is linked into `~/.codex/agents/`, and each directory in `config/codex/skills/` is linked into `~/.codex/skills/`
