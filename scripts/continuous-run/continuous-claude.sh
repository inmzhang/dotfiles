...
usage() {
  cat <<'EOF'
Usage: continuous-claude.sh [options]

Runs Claude Code in a persistent implementation loop against a target
repository. Launch it from inside the target repo or pass `--repo PATH`.

Options:
  --repo PATH            Target git repository. Default: current working tree.
  --once                 Run exactly one Claude round and exit.
  --reset-session        Discard any saved session id and start fresh.
  ...
  --max-failures COUNT   Stop after this many failed Claude rounds. Default: 1
  --parallel             Enable prompt guidance for Claude parallel agent teams.
  --safe                 Use `--permission-mode auto` instead of bypass mode.
  --help                 Show this help.
...
Environment overrides:
  ...
  CLAUDE_STATE_DIR              State directory. Default: <repo>/.claude-loop
  CLAUDE_PROMPT_FILE            Prompt template path.
  CLAUDE_INTERVAL               Sleep between rounds.
  CLAUDE_MAX_IDLE               Max unchanged rounds before stopping.
  CLAUDE_MAX_FAILURES           Max failed rounds before stopping.
  CLAUDE_EXEC_MODE              `dangerous` (default) or `safe`
  CLAUDE_LOOP_MODEL             Optional model passed to Claude.
  CLAUDE_EXTRA_FLAGS            Extra flags appended to Claude commands as a shell-style string.

Stop mechanism:
  Touch <repo>/.claude-loop/STOP while the loop is running.
EOF
}

PROMPT_FILE="${CLAUDE_PROMPT_FILE:-$SCRIPT_DIR/prompts/continuous-implementation.md}"
INTERVAL="${CLAUDE_INTERVAL:-5}"
MAX_IDLE="${CLAUDE_MAX_IDLE:-3}"
MAX_FAILURES="${CLAUDE_MAX_FAILURES:-1}"
EXEC_MODE="${CLAUDE_EXEC_MODE:-dangerous}"
...
STATE_DIR="${CLAUDE_STATE_DIR:-$REPO_ROOT/.claude-loop}"
...
case "$EXEC_MODE" in
  ...
  safe)
    ...
    ;;
  *)
    printf 'Unsupported CLAUDE_EXEC_MODE: %s\n' "$EXEC_MODE" >&2
    exit 1
    ;;
esac

EXTRA_FLAGS=()
if [[ -n "${CLAUDE_LOOP_MODEL:-}" ]]; then
  EXTRA_FLAGS+=(--model "$CLAUDE_LOOP_MODEL")
fi
if [[ -n "${CLAUDE_EXTRA_FLAGS:-}" ]]; then
  # Intentional word splitting for user-provided extra CLI flags.
  # shellcheck disable=SC2206
  USER_EXTRA_FLAGS=(${CLAUDE_EXTRA_FLAGS})
...
PARALLEL_PROMPT="$(continuous_run_parallel_prompt claude "$PARALLEL_ENABLED")"
...
run_claude_round() {
  local round_no="$1"
  local prompt_text text_log
  prompt_text="$(compose_prompt "$round_no")"
  text_log="$STATE_DIR/logs/round-$(printf '%04d' "$round_no").txt"
  ...
  if [[ -f "$SESSION_FILE" ]]; then
    ...
    if ! printf '%s\n' "$prompt_text" | \
      claude --print \
        ...
        "${EXTRA_FLAGS[@]}" \
        > "$text_log" 2>&1; then
      [[ -f "$text_log" ]] && cp "$text_log" "$STATE_DIR/latest.txt"
      return 1
    fi
  else
    local session_id
  ...
  cp "$text_log" "$STATE_DIR/latest.txt"
}
...
while true; do
  ...
  if run_claude_round "$round"; then
    failure_count=0
    printf 'Session: %s\n' "$(cat "$SESSION_FILE")"
    printf 'Output tail:\n'
    tail -20 "$STATE_DIR/latest.txt"
    printf '\n'
    ...
    printf 'Claude round %d failed (%d/%d)\n' "$round" "$failure_count" "$MAX_FAILURES" >&2
    if [[ -f "$STATE_DIR/latest.txt" ]]; then
      printf 'Output tail before failure:\n' >&2
      tail -20 "$STATE_DIR/latest.txt" >&2
      printf '\n' >&2
    fi
